...
## What's in the Repo
The first part of the project isn't in this repo (since it needed to be a CSLib PR). The main attraction in this repo is the `GrindAttrLint` folder, which has all my work on `#grind_attr_lint`. A lot of the code is copy-pasted from Lean's own `#grind_lint` code, most of the work of this project was in understanding that code and metaprogramming in general to the point that I could make changes to it. Along the way I produced a `#print_attr` tactic, which lives in `PrintAttr`.
